...
This code is released under the MIT License (see LICENSE). We acknowledge Google Earth Engine and its public data archive, and thank the providers of the open-source datasets: NASA (MODIS, SRTM), ECMWF (ERA5 reanalysis), and ISRIC/iSDA (soil maps). We also appreciate the open-source Python ecosystem (NumPy, SciPy, PyTorch, etc.) used in this work. Special thanks to thesis advisors and collaborators for guidance on the research design and domain expertise.
